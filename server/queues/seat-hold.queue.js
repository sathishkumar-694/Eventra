...
seatHoldQueue.on("error", (err) => {
  console.error("Seat hold queue connection error:", err.message);
});

if (process.env.ENABLE_WORKERS === "true") {
  const seatHoldWorker = new Worker(
    QUEUE_NAME,
    async (job) => {
  ...
  seatHoldWorker.on("error", (err) => {
    console.error("Seat hold worker connection error:", err.message);
  });
